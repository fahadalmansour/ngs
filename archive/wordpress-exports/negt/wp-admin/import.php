...
if ( empty( $importers ) ) {
	if ( current_user_can( 'install_plugins' ) ) {
		...
		echo '<p>' . sprintf(
			...
			__( 'No importers are available. You can <a href="%s">search for and install importer plugins</a> to import content from other platforms.' ),
			$search_url
		) . '</p>';
	} else {
		echo '<p>' . __( 'No importers are available. Please contact your site administrator to install importer plugins.' ) . '</p>';
	}
} else {
