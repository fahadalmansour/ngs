...
Based on: /Users/fahadalmansour/Downloads/import_products_example_2025.xlsx
Fills all 111 columns with proper data from WooCommerce.
"""
import os
import re
...
def get_attribute_value(attributes, attr_name):
    """Get attribute value by name (e.g., 'Brand', 'Compatibility')."""
    if not attributes:
        return ''
...
def export_to_zid_xlsx(products):
    ...
    # All 111 Zid.sa columns from template
    columns = [
    ...
    for p in products:
        ...
        short_desc = clean_html(p.get('short_description', ''))

        # === ATTRIBUTES FOR FILTRATION ===
        ...
        row = {
            ...
            # Keywords & Description (22-31)
            'keywords': tags if tags else name,
            'description_ar': description[:5000] if description else name,
            'description_en': description[:5000] if description else name,
            'short_description_ar': short_desc[:1000] if short_desc else name,
            'short_description_en': short_desc[:1000] if short_desc else name,
            'product_page_title_ar': name,
            'product_page_title_en': name,
            'product_page_description_ar': short_desc[:160] if short_desc else name,
            'product_page_description_en': short_desc[:160] if short_desc else name,
            'product_page_url': '',

            # Product Options (32-44) - No variants
            'has_variants': 'No',
            'option1_name_ar': '',
            'option1_name_en': '',
            'option1_value_ar': '',
            ...
            # Product Additions (45-96) - All No
            'has_dropdown': 'No',
            ...
            'dropdown_choice3_ar': '',
            'dropdown_choice3_en': '',
            'dropdown_choice3_price': '',
            'has_text_input': 'No',
            'is_text_input_required': '',
            'text_input_name_ar': '',
            'text_input_name_en': '',
            'text_input_price': '',
            'has_multiple_options': 'No',
            'is_multiple_options_required': '',
            'multiple_options_name_ar': '',
            'multiple_options_name_en': '',
            'multiple_options_choice1_ar': '',
            'multiple_options_choice1_en': '',
            'multiple_options_choice1_price': '',
            'multiple_options_choice2_ar': '',
            'multiple_options_choice2_en': '',
            'multiple_options_choice2_price': '',
            'multiple_options_choice3_ar': '',
            'multiple_options_choice3_en': '',
            'multiple_options_choice3_price': '',
            'has_numerical_input': 'No',
            ...
            # Filtration Attributes (97-110) - Brand & Compatibility
            ...
            'filtration_type_value_1_ar': '',
            'filtration_type_value_1_en': '',
            'filtration_type_1': '',
            ...
            'filtration_type_value_2_ar': '',
            'filtration_type_value_2_en': '',
            'filtration_type_2': '',
        }
    ...
    filled = {k: v for k, v in sample.items() if v}
    print(f"Filled columns: {len(filled)} / {len(columns)}")
    for k, v in list(filled.items())[:15]:
        print(f"  {k}: {str(v)[:60]}")
...
def main():
    print("=" * 60)
    print("WooCommerce to Zid.sa XLSX Exporter (ALL COLUMNS)")
    print("=" * 60)
    ...
    print("3. Upload: zid_import_products.xlsx")
    print("4. Review and confirm the import")
    print("=" * 60)
