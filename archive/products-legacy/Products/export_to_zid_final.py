...
"""
Export WooCommerce products to Zid.sa XLSX format - FINAL VERSION.
Uses WooCommerce API data (product-specific) since website has only template content.
"""
import os
import re
import sys
...
def export_to_zid_xlsx(products):
    """Export products to Zid.sa XLSX format with ALL data."""
    ...
    for i, p in enumerate(products):
        ...
        # API descriptions are product-specific: "{Name} available at NGS Smart Home Store. Price: X SAR."
        description = clean_html(p.get('description', ''))
        short_desc = clean_html(p.get('short_description', ''))

        # Fallback if API descriptions are empty
        if not description or len(description) < 10:
            description = f"{name} متوفر في متجر نيوجين للمنزل الذكي. السعر: {price} ريال سعودي."
        if not short_desc or len(short_desc) < 10:
            short_desc = f"{name} - منتج من نيوجين"
        ...
        row = {
            ...
            # Keywords & Description - From WooCommerce API
    ...
    if rows:
        ...
        desc_preview = rows[0]['description_ar'][:100].replace('\n', ' ')
        print(f"  Description: {desc_preview}...")
        print(f"  Short desc: {rows[0]['short_description_ar'][:60]}...")
...
def main():
    print("=" * 60)
    print("WooCommerce to Zid.sa XLSX Exporter (FINAL)")
    print("=" * 60)
